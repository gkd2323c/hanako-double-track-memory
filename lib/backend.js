 ...
 */
async function openFactsDb(factsDbPath, log) {
  ...
  try {
    ...
    // 修复 silent failure：把真实错误暴露到 stderr + log 两渠道
    // 原因：toolCtx.log 链路里 log 可能是 undefined（platform 没有传 log 给 tool execute 上下文时）
    // 用 console.error 兑底，确保 platform 任意路径下都能被看到
    console.error(`[double-track-memory] facts.db open failed: ${e.code || ""} ${e.message} | factsDbPath=${factsDbPath} | cwd=${process.cwd()}`);
    if (log && typeof log.warn === "function") {
      log.warn(`[double-track-memory] facts.db open failed: ${e.code || ""} ${e.message}`);
    }
    return null;
  }
}
...
export async function searchFacts({ keywords, minKeywordHits, tags, limit, factsDbPath, log }) {
  ...
  const db = await openFactsDb(factsDbPath, log);
  if (!db) {
    return {
      ok: true,
      degraded: true,
  ...
  try {
    ...
    const sql = `
      ...
      LIMIT ?2
    `;
    // 修复 placeholder 数量错配：SQL 里有 3 个 ?（JOIN、subquery、LIMIT），但 better-sqlite3
    // 按出现顺序 binding，重复 ?1 需要传两次。改用 named placeholder 避免歧义。
    // 同时：fts_hit 子查询始终返回 1（outer JOIN 已限定 FTS 命中），移除以简化 SQL。
    const sql2 = `
      SELECT
        f.id,
        f.fact,
        ...
        bm25(facts_fts) AS bm25_score
      FROM facts f
      JOIN facts_fts(@fts) ON facts_fts.rowid = f.id
      ORDER BY bm25_score ASC, f.created_at DESC
      LIMIT @lim
    `;
    rows = db.prepare(sql2).all({ fts: ftsQuery, lim: limit * 3 });
    // bm25() 在 FTS5 里只对 "匹配 query 的 row" 有意义。fts_hit = 实际命中的关键词数（程序算）
    // 去掉子查询：outer JOIN 已限定 FTS 命中，多个关键词是否都命中用 lower.includes() 数
    for (const r of rows) {
